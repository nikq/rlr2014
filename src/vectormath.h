...
inline void reflect( const Vector& I, const Vector& N, Vector& result ){
  result = I - N * 2.f * N.dot( I );
}
inline bool refract( const Vector& I, Vector& N, float eta, Vector& result ){
  if( I.dot( N ) > 0.f )
    N = N * -1.f;
  float cosi  = - I.dot( N ); // dot(-i, n);
  float cost2 = 1.0f - eta * eta * (1.0f - cosi * cosi);
  if( cost2 < 0.f ) return false; // 全反射.
